...
export default function ClienteRutasAdquiridas() {
  ...
  const cargarRutasCompradas = async () => {
    try {
      ...
      if (userDoc.exists()) {
        const rutas = userDoc.data().rutasCompradas || [];
        console.log('Rutas compradas:', rutas);
        const timestamp = Date.now();
        const rutasConId = rutas.map((ruta, index) => ({
          ...ruta,
          uniqueId: ruta.rutaId || `ruta-${user.uid}-${index}-${timestamp}`
        }));
        setRutasCompradas(rutasConId);
      }
    } catch (error) {
        ...
        {rutasCompradas.length === 0 ? (
          ...
          <div className="space-y-6">
            {rutasCompradas.map((ruta) => (
              <div key={ruta.uniqueId} className="bg-white rounded-lg shadow-lg p-6">
